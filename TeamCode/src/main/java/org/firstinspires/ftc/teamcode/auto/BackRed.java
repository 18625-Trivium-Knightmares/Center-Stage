...
import org.firstinspires.ftc.teamcode.redBack.Right;
import org.firstinspires.ftc.teamcode.redBack.Mid;
import org.firstinspires.ftc.teamcode.redBack.Left;
...
@Autonomous (group = "AUTO", name = "Red Back Stage")
@Config
//@Disabled
public class BackRed extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
         ...
         * Right case
         */
        Trajectory RIGHT_PURPLE_PIXEL = drive.trajectoryBuilder(new Pose2d(Robot.RB_STARTX, Robot.RB_STARTY, Math.toRadians(Robot.RB_START_HEADING)))
                .addTemporalMarker(0.1, () -> {
                    robot.servoPosition("piv", 0.5);
                    robot.setChainTarget(-1540.75, 0.5);
                })
                .splineToConstantHeading(new Vector2d(Right.PURPLE_PIX_X, Right.PURPLE_PIX_Y), Math.toRadians(Right.PURPLE_PIX_TANG))
        ...
        Trajectory RIGHT_YELLOW_PIXEL = drive.trajectoryBuilder(RIGHT_PURPLE_PIXEL.end(), true)
                .splineToLinearHeading(new Pose2d(Right.YELLOW_PIX_X, Right.YELLOW_PIX_Y, Math.toRadians(Right.YELLOW_PIX_HEAD)), Math.toRadians(Right.YELLOW_PIX_TANG))
                .addDisplacementMarker(() -> {
         ...
         * Middle case
         */
        Trajectory MID_PURPLE_PIXEL = drive.trajectoryBuilder(new Pose2d(Robot.RB_STARTX, Robot.RB_STARTY, Math.toRadians(Robot.RB_START_HEADING)))
                .addDisplacementMarker(() -> {
                    robot.servoPosition("piv", 0.5);
                    robot.setChainTarget(-1540.75, 0.5);
                })
         ...
         * Left case
         */
        Trajectory LEFT_PURPLE_PIXEL = drive.trajectoryBuilder(new Pose2d(Robot.RB_STARTX, Robot.RB_STARTY, Math.toRadians(Robot.RB_START_HEADING)))
                .splineToSplineHeading(new Pose2d(Left.PURPLE_PIX_X, Left.PURPLE_PIX_Y, Math.toRadians(Left.PURPLE_PIX_HEAD)), Math.toRadians(Left.PURPLE_PIX_TANG))
        ...
        Trajectory LEFT_YELLOW_PIXEL = drive.trajectoryBuilder(LEFT_PURPLE_PIXEL.end(), true)
                ...
                .splineToSplineHeading(new Pose2d(Left.YELLOW_PIX_X, Left.YELLOW_PIX_Y, Math.toRadians(Left.YELLOW_PIX_HEAD)), Math.toRadians(Left.YELLOW_PIX_TANG))
                .addDisplacementMarker(() -> {
                    robot.servoPosition("claw 2", Robot.openClaw2);
                })
                .build();
